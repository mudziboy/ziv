...
func main() {
	// Membaca API Key dari file sistem
	if keyBytes, err := ioutil.ReadFile(ApiKeyFile); err == nil {
		AuthToken = strings.TrimSpace(string(keyBytes))
	}

	// Membaca Port dari file sistem (Default ke 8888 jika file tidak ada)
	portStr := "8888"
	...
	log.Printf("ZiVPN API Management started at :%s", portStr)
...
func createUser(w http.ResponseWriter, r *http.Request) {
	...
	users, _ := loadUsers()
	users = append(users, UserStore{Password: req.Password, Expired: expDate, Status: "active"})
	saveUsers(users)

	// Restart engine ZiVPN untuk menerapkan user baru
	exec.Command("systemctl", "restart", "zivpn.service").Run()

	jsonResponse(w, http.StatusOK, true, "User ZiVPN berhasil dibuat", map[string]string{
